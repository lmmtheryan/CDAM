...
function finalLabel = GCAlgo( im, fixedBG, param_GC)

K=param_GC.K;
G=param_GC.G;
maxIterations=param_GC.maxIterations;
Beta=param_GC.beta;
diffThreshold=param_GC.diffThreshold;
...
for iter=1:maxIterations
    ...
    fgLogPL(fixedBG==1) = max(max(fgLogPL));
%     fgLogPL=max(max(fgLogPL))*ones(size(fgLogPL));
